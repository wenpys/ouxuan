...
public class CursorUtils {
	...
	// OneToManyLazyLoader is a raw type. References to generic type
	// OneToManyLazyLoader<O,M> should be parameterized
	// Type safety: The constructor OneToManyLazyLoader(Object, Class, Class,
	// FinalDb) belongs to the raw type OneToManyLazyLoader. References to
	// generic type OneToManyLazyLoader<O,M> should be parameterized
				...
				if (columnCount > 0) {
					...
					for (ManyToOne manyToOneProp : table.manyToOneMap.values()) {
						if (manyToOneProp.getDataType() == ManyToOneLazyLoader.class) {
							...
							manyToOneLazyLoader.setFieldValue(cursor
									.getInt(cursor.getColumnIndex(manyToOneProp
											.getColumn())));
	...
	public static <T> T dbModel2Entity(DbModel dbModel, Class<?> clazz) {
		if (dbModel != null) {
			...
			try {
				...
				// Unchecked cast from capture#5-of ? to T
				T entity = (T) clazz.newInstance();

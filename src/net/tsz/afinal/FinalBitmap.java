...
public class FinalBitmap {
	...
	// * @return
	// */
	// public FinalBitmap configRoundImg(boolean isRound){
	// mConfig.defaultDisplayConfig.isRound = isRound;
	// return this;
	// }
	...
	public void display(View imageView, String uri) {
		doDisplay(imageView, uri, null, false);
	}

	public void display(View imageView, String uri, int imageWidth,
			int imageHeight, boolean isRound) {
		...
		doDisplay(imageView, uri, displayConfig, isRound);
	}
	...
	public void display(View imageView, String uri, Bitmap loadingBitmap) {
		...
		doDisplay(imageView, uri, displayConfig, false);
	}

	public void display(View imageView, String uri, Bitmap loadingBitmap,
			Bitmap laodfailBitmap, boolean isRound) {
		BitmapDisplayConfig displayConfig = configMap.get(String
	...
	public void display(View imageView, String uri, BitmapDisplayConfig config) {
		doDisplay(imageView, uri, config, false);
	}

	@SuppressWarnings("deprecation")
	private void doDisplay(View imageView, String uri,
			BitmapDisplayConfig displayConfig, boolean isRound) {
		if (!mInit) {
			init();
		}
		...
		if (bitmap != null) {
			viewSetImage(imageView, bitmap, displayConfig, isRound);
			...
			task.executeOnExecutor(bitmapLoadAndDisplayExecutor, uri, isRound);
		}
	}
	...
	private Bitmap processBitmap(String uri, BitmapDisplayConfig config) {
		if (mBitmapProcess != null) {
			return mBitmapProcess.getBitmap(uri, config, mContext);
		}
		return null;
	}
	...
	private class BitmapLoadAndDisplayTask extends
		...
		public BitmapLoadAndDisplayTask(LoadBitmapCallback callback,
				BitmapDisplayConfig config) {
			displayConfig = config;
			...
			if (imageViewReference != null) {
				...
				if (bitmap != null && imageView != null) {
					mConfig.displayer.loadCompletedisplay(imageView, bitmap,
							displayConfig, mFinalBitmap, isRound);
				} else if (bitmap == null && imageView != null) {
					mConfig.displayer.loadFailDisplay(imageView,
							displayConfig.getLoadfailBitmap(), mFinalBitmap,
							isRound);
				}
			} else {
				callback.doSomething(bitmap);
			}
		}
	...
	public void loadBmpWithConfig(String uri, int imageWidth, int imageHeight,
		...
		loadBmpWithConfig(uri, displayConfig, callback);
	}
	 ...
	 *            当图片加载成功后调用接口方法
	 */
	public void loadBmpWithConfig(String imageUrl, LoadBitmapCallback callback) {
		loadBmpWithConfig(imageUrl, null, callback);
	}
	public void loadBmpWithConfig(String imageUrl,
			BitmapDisplayConfig displayConfig, LoadBitmapCallback callback) {
		loadBmpWithWH(imageUrl, displayConfig, callback);
	}
	 ...
	 *            当图片加载成功后调用接口方法
	 */
	public void loadBmpWithWH(String uri, BitmapDisplayConfig displayConfig,
			LoadBitmapCallback callback) {
		if (!mInit) {
			init();
		}
		...
		if (bitmap != null) {
			...
			final BitmapLoadAndDisplayTask task = new BitmapLoadAndDisplayTask(
					callback, displayConfig);
			task.executeOnExecutor(bitmapLoadAndDisplayExecutor, uri, false);
		}
	}
	 ...
	 * 添加缓存
	 */
	public void addMemoryCache(String dataString, Bitmap bitmap) {
		if (mImageCache != null)
			mImageCache.addToMemoryCache(dataString, bitmap);
	}
}

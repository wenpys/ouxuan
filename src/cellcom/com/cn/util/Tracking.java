...
public class Tracking {
	...
	public static List<CharSequence> getFlowSet(Context context) {
		CharSequence[] items = context.getResources().getStringArray(
				getMetaDataBundle(context).getInt("flow"));
		Arrays.asList(items);
	...
	public static String getNetworkConnection(Context context) {
		...
		return FlowConsts.DISCONNECTED;
	}

	public static String getCarrier(Context context) {
		TelephonyManager telephonyManager = (TelephonyManager) context
				.getSystemService(FlowConsts.PHONE);
		String carrier = "";
		String networkOperator = telephonyManager.getNetworkOperator();
		if ((networkOperator.equals(FlowConsts.CHINA_MOBILE_46000))
				|| (networkOperator.equals(FlowConsts.CHINA_MOBILE_46002)))
			carrier = FlowConsts.CHINA_MOBILE;
		else if (networkOperator.equals(FlowConsts.CHINA_UNICOM_46001))
			carrier = FlowConsts.CHINA_UNICOM;
		else if (networkOperator.equals(FlowConsts.CHINA_TELECOM_46003))
			carrier = FlowConsts.CHINA_TELECOM;
		else {
	...
	public static String[] getAppVersioin(Context context) {
		...
		try {
			...
			// //versionCode=versionCode+"."+versionName.substring(0,
			// versionName.indexOf("-"));
			// versionCode=versionName.substring(0, versionName.indexOf("-"));
			// prodate=versionName.substring(
			// versionName.indexOf("-")+1,versionName.length());
			// }
	...
	public static String getApplicationId(Context context) {
		try {
			...
			LogMgr.showLog(e.toString());
		} catch (PackageManager.NameNotFoundException e) {
			LogMgr.showLog("getApplicationId.NameNotFoundException"
					+ e.toString());
		}
		return FlowConsts.UNKNOWN_APP_ID;
	}
	...
	public static String getClientKey(Context context) {
		try {
			...
			LogMgr.showLog("getClientKey.NameNotFoundException" + e.toString());
		}
		return FlowConsts.UNKNOWN_CLIENT_KEY;
	}
	...
	public static synchronized long getTimeStamp() {
		long seconds = System.currentTimeMillis();// 鏃堕棿鎴�
		return seconds;
	}
	...
	public static Bundle getMetaDataBundle(Context context) {
		try {
			...
			LogMgr.showLog("getMetaData NameNotFoundException = "
					+ e.toString());
		}
		return null;
	}
